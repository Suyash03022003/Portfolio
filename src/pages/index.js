...
import { BarlowCondensed_400Regular, BarlowCondensed_500Medium, BarlowCondensed_700Bold } from "@fontsource/barlow-condensed";
...
export default function Home() {
  ...
  const downloadResume = () => {
    ...
    link.download = 'Suyash_Patalbansi_Resume.pdf';
  ...
  return (
    <>
      <div className={`fixed top-0 left-0 h-screen flex gap-0 items-strech z-50 ${!showNav ? 'w-0' : 'w-full'}`}>
        <div className='relative w-[65%] h-screen bg-black/80 flex flex-col gap-7 items-center justify-center shadow-2xl shadow-[#bfbebd] z-50'>
          ...
          {['Home', 'About Me', 'Skills', 'Education', 'Projects', 'Contact Me'].map((link, index) => (
            <p key={index} className={`relative cursor-pointer text-white font-semibold tracking-[2px] uppercase pb-1.5 overflow-hidden myLink ${currentIndex === index ? 'font-bold text-2xl activeNav' : 'text-xl hover:scale-105'} ${!showNav ? 'hidden' : 'block'}`} onClick={() => handleMouseScroll(index)} style={mainHeadingStyles}>{link}</p>
          ))}
        </div>
        <div className="w-[35%] h-screen bg-gradient-to-r from-black/70 via-90% to-black/40"></div>
      </div>
      <p className={`fixed text-white text-xl font-semibold rounded-full bg-white/30 flex items-center justify-center cursor-pointer hover:scale-110 ${showNavButton ? 'w-14 h-14 top-7 left-7' : 'w-0 h-0 top-0 left-0'} z-40`} onClick={() => { setShowNav(true) }}>
        <FontAwesomeIcon icon={faBars} className={`${showNavButton ? 'block' : 'hidden'} w-6 h-6`} />
      </p>
      <main id="Hero" className={` w-[100%] h-screen bg-gradient-to-r from-[#0d0d0d] to-50% to-[#303030] flex items-center pr-[50px] overflow-hidden ${showNav ? 'blur' : ''}`} ref={(ref) => sectionRefs.current[0] = ref}>
        <div className="pl-[20px] w-[79%] h-screen flex flex-col items-start justify-between pt-[20px]">
          <div className={`flex items-center gap-8 my-5 pl-[40px] ${!showNavButton ? 'visible' : 'invisible'}`}>
            {['Home', 'About Me', 'Skills', 'Education', 'Projects', 'Contact Me'].map((link, index) => (
              <p key={index} className={`relative cursor-pointer text-white font-semibold tracking-[2px] uppercase pb-1.5 overflow-hidden myLink ${currentIndex === index ? 'font-bold text-xl activeNav' : 'hover:scale-105 text-lg'}`} onClick={() => handleMouseScroll(index)} style={mainHeadingStyles}>{link}</p>
            ))}
          </div>
          <div
            className='absolute bottom-24 flex items-center justify-center left-1/2 cursor-pointer z-[100]'
            id='downButton'
          ...
          <div className="pl-[40px] w-full py-[80px] flex flex-col items-start justify-end gap-7 z-20">
      ...
      <main id="AboutMe" className={`w-full bg-gradient-to-r from-[#0d0d0d] to-50% to-[#303030] overflow-hidden ${showNav ? 'blur' : ''} pb-10`} ref={(ref) => sectionRefs.current[1] = ref}>
        ...
        <div className='w-3/5 m-auto mt-8 flex flex-col items-start gap-3 text-justify'>
          ...
          <p className='text-[#f1f1f1] text-xl leading-9 tracking-[1px] mt-5'>
            <span className='block font-semibold text-white text-2xl text-center mb-2'>My Interests:</span>
          ...
          <p className='text-[#f1f1f1] text-xl leading-9 tracking-[1px] mt-5'>
            <span className='block font-semibold text-white text-2xl text-center mb-2'>Let's Connect:</span>
          ...
          <div className='w-full flex items-center justify-between mt-5'>
            ...
            <div className='flex items-center gap-4'>
              <a href="https://github.com/Suyash03022003" target="_blank" rel="noopener noreferrer" className='w-14 h-14 rounded-full border-2 flex items-center justify-center'>
                <FontAwesomeIcon icon={faGithub} className="text-white text-3xl hover:text-gray-500" />
              </a>
              <a href="https://www.linkedin.com/in/suyash-patalbansi" target="_blank" rel="noopener noreferrer" className='w-14 h-14 rounded-full border-2 flex items-center justify-center'>
                <FontAwesomeIcon icon={faLinkedin} className="text-white text-3xl hover:text-blue-600" />
              </a>
              <a href="https://www.instagram.com/suyashpatalbansi/" target="_blank" rel="noopener noreferrer" className='w-14 h-14 rounded-full border-2 flex items-center justify-center'>
                <FontAwesomeIcon icon={faInstagram} className="text-white text-3xl hover:text-[#fc035a]" />
              </a>
            </div>
          </div>
        </div>
      ...
      <main id="Education" className={`w-full bg-gradient-to-r from-[#0d0d0d] to-50% to-[#303030] overflow-hidden ${showNav ? 'blur' : ''} pb-10`} ref={(ref) => sectionRefs.current[3] = ref}>
        <div className="w-full flex items-center justify-center gap-5 mt-20">
          <div className='w-1/4 h-0.5 bg-white/80'></div>
      ...
      <main id="AboutMe" className={`w-[100%] h-screen bg-gradient-to-r from-[#0d0d0d] to-50% to-[#303030] flex items-center pr-[50px] overflow-hidden ${showNav ? 'blur' : ''}`} ref={(ref) => sectionRefs.current[5] = ref}></main>
    </>
  );
}
